...
<script>
	...
	<body id="body">
		...
		<a href="shop_update.jsp?code=<%=code %>&price=<%=price %>&content=<%=content %>&pstock=<%=pstock %>" style="font-weight:bold; color:black; float: right;">상품수정</a><br>
		<%}else{}%>
			<table border="1">
			<tr>
			...
			<tr>
			<td><input type="hidden" id="sstock" value=<%=pstock %>><br><input type="button" id="b1" value="장바구니"></td>
			<td><br><input type="button" id="b2" value="바로 구매"></td>
			</tr>
			...
			<hr>
				...
				<form action="cdelete">
						<c:if test="${vo.code eq code}">
							${vo.scomment} <input type="hidden" name="code" value=<%=code %>><input type="hidden" name="scomment" value=${vo.scomment}>
							<input name="scpw" style="float: right;" value="비밀번호를 입력해주세요" onFocus="this.value=''; return true;">
							<input type="submit" style="float: right;" value="삭제"><br><hr><br>
						</c:if>
				</form>
					</c:forEach>
<form action="ccreate">
<input type="hidden" name="code" value=<%=code %>>
						<textarea rows="5" cols="110" name="scomment"></textarea><br>
						비밀번호 : <input name="scpw">
						<input type="submit" style="float: right;" value="등록"><br>
					<hr>
					</form>
		</div>
	</body>
