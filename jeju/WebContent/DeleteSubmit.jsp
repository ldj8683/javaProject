...
<%
//BoardDelete.jsp에서 입력된 값 가져오기!
String s1 = request.getParameter("no"); // 입력값은 모두 스트링이다!
int no = Integer.parseInt(s1); // 스트링으로 입력 받은 값을 int로 캐스팅!

String id = (String)session.getAttribute("id");

notice_board nb = new notice_board(); // delete메서드 사용을 위한 new(틀) 생성
nb.delete(no); // 메서드 사용!
...
<html>
	...
	<body>
		...
		<nav id="Content"> <!-- 본문, 각 조원들이 넣게될 본문 레이아웃 생성 -->
			삭제되었습니다.
			<form action="ServiceMain.jsp"><!-- 삭제 후 돌아가기 -->
				<button>돌아가기</button>
			</form>
		</nav> <!-- 본문 닫음 -->
	</body>
