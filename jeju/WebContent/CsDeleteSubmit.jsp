...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    <!-- 삭제파트에서 넘어온 입력값(게시글번호) 받기 -->
    <%
    // CsDelete에서 입력한 값을 받아 줘야 한다!
    String s1 = request.getParameter("customer_no");
    int customer_no = Integer.parseInt(s1);
    //입력값은 스트링이기 때문에 인트로 캐스팅
    
    String id = (String)session.getAttribute("id");
    
    CustomerSoundDB db = new CustomerSoundDB(); // delete 메서드 사용 new
    db.delete(customer_no); // 메서드 사용
    %>
...
<html>
	...
	<body>
		...
		<nav id="Content"> <!-- 본문, 각 조원들이 넣게될 본문 레이아웃 생성 -->
			삭제되었습니다.
			<form action="CsMain.jsp"> <!-- 삭제 처리 후 돌아가기 -->
				<button>돌아가기</button>
			</form>
		</nav> <!-- 본문 닫음 -->
	</body>
