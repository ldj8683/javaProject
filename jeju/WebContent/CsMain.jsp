...
<%
	...
	// list 메서드를 사용하기 위해서 new를 만들어 준다
	CustomerSoundDB db = new CustomerSoundDB();
	ArrayList<CustomerSoundVO> list = db.list();
	// list메서드를 사용하기 위해서 arraylist의 참조형 변수를 만들어 준다!
...
<html>
	...
	<body>
		...
		<hr color="blue">
			...
			<!-- 노출되는 값이 여러 row값이기 때문에 for문을 돌려준다! -->
			<%
				for (int i = 0; i < list.size(); i++) {
				CustomerSoundVO bag = list.get(i);
				if (bag.getCustomer_type().equals("좋아요")) {
			%><!-- list에서 가져온 bag에서 값을 빼야한다! -->
			<tr>
				<td><%=bag.getCustomer_no()%></td>
				<td><a href="Csdetail.jsp?customer_no=<%=bag.getCustomer_no()%>"><%=bag.getCustomer_title()%></a></td>
				...
				<td><button onclick ="location.href='CsDelete.jsp'">삭제</button></td>
			</tr>
			<%
				}
			%>
		...
		<table>
			...
			<%
				for (int i = 0; i < list.size(); i++) {
				CustomerSoundVO bag = list.get(i);
				if (bag.getCustomer_type().equals("나빠요")) {
			%><!-- list에서 가져온 bag에서 값을 빼야한다! -->
			<tr>
				<td><%=bag.getCustomer_no()%></td>
				<td><a href="Csdetail.jsp?customer_no=<%=bag.getCustomer_no()%>"><%=bag.getCustomer_title()%></a></td>
