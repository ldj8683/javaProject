...
//	request.setCharacterEncoding("utf8");
	// ProductRegister에서 입력되어서 넘어온 파라미터들을 각각의 변수들에 저장합니다.
	String productClassification = request.getParameter("productClassification");
	String productName = request.getParameter("productName");
	String productImage = request.getParameter("productImage");
	int productPrice = Integer.parseInt(request.getParameter("productPrice"));/* 입력받은 데이터는 String이기 때문에 정수형으로 캐드싵을 해줍니다. */
	String productLocation = request.getParameter("productLocation");
	/* productCommon과 productInternal은 체크박스로 중복으로 값을 받도록 설정을 했기 때문에 배열로써 파리미터값들을 받았습니다. */
	...
	// 로그인에 성공을하면 로그인이 계속 되어있게 하기 위해서 세션을 설정해주었습니다. 또한, id를 출력을 할 경우가 있기 때문에 문자열 변수 id를 저장해 주었습니다.
	String id = (String)session.getAttribute("id");
	
	// productCommon과 productInternal 둘 모두 배열로써 파라미터 값들을 받았습니다.
	// 이것을 db에 저장을 하기 위해서 하나의 String으로 합쳐주고 ','로 구분을 주어서 저장을 했습니다.
	...
	// 입력받은 파라미터 값들을 각각에 변수로 저장을 한 후 이것을 하나로 묶어서주기 위해서 bag라는 변수에 넣어주었습니다.
	...
	// 저장된 bag를 db로 보내기 위해서 db 객체를 생성해주고, 이를 이용해서 create 함수를 사용할 수 있도록 했습니다.
	...
	<body>
		...
		<nav id="Content" style="background:#16b4f7;"> <!-- 각 조원들이 넣게될 본문 레이아웃 생성 -->
			<div style="padding-top: 4em; padding-left: 4em;">
				숙소 정보 등록이 완료 되었습니다.<br><br>
				<a href="ProductSummary.jsp?classification=<%= bag.getProductClassification() %>"> <!-- 저장을 한 후 다시 요약으로 돌아가기 위한 버튼을 생성해 주었습니다. -->
					<button>돌아가기</button>
				</a>
			</div>
		</nav> <!-- 본문 닫음 -->
